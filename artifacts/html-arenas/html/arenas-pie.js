...
//
// Labels: emoji + percent inside the slice when it fits (>= 10%, ~36° at this
// radius); smaller slices get a legend line under the pie instead — inline
// text in a sliver would be cramped or overflow the wedge.
//
...
(function () {
  ...
  window.buildSportPie = function (breakdown, colors, narrow) {
    ...
    if (data.length === 1) {
      ...
      data.forEach(function (d) {
        ...
        if (d.pct >= 10) {
          ...
          parts.push('<text x="' + lx.toFixed(2) + '" y="' + (ly + 4).toFixed(2) + '" text-anchor="middle" font-size="12" font-weight="700" fill="white">' + sc.icon + ' ' + d.pct + '%</text>');
        } else {
          legend.push(
    ...
    var size = narrow ? '132px' : '150px';
    return (
      '<div style="' + (narrow
        ? 'border-top:var(--border);padding:14px;display:flex;flex-direction:column;align-items:center;gap:8px'
        : 'width:210px;flex-shrink:0;border-left:var(--border);padding:14px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px') + '">' +
