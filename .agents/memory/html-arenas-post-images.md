...
- Feed scope RESOLVED: legacy unscoped `GET /api/posts` DELETED (do not reintroduce an unscoped post list — comment guards the spot). Main feed posts = buildFeedPosts (follows + self) only; club surfaces stay membership-scoped by user decision ("club is its own opt-in sharing context", same as activities/events). `activity_feed_visible` deliberately covers activities only (label-accurate; user rejected extending to posts). Route audit found no other unscoped+uncalled data routes.
