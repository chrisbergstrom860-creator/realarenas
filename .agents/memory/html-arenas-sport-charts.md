...
- In-slice pie %: white 12u bold label only when arc length at label radius 50 (of 80) >= chars*7.2+6 viewBox units (~9-10%); legend always carries every sport, so omission loses nothing. White clears AA on every accent — asserted in verify-sport-charts.js. Bar plot is 200u tall, slots 40/10 desktop vs 30/6 narrow (keeps 12-sport charts from over-shrinking on phones); pie renders 220px.
