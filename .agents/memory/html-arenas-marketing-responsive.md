...
- about = "story" layout: hero + 3 `.story` sections (`.story-num` + `.story-title`) + dark `.cta-band`. terms & privacy = shared "legal" layout: hero + `.prose.legal` with `.legal-section` dividers, `.legal-title` headings, bold `<strong>` list lead-ins, mono `.doc-meta` "Last updated" line; NO cta-band (a legal doc shouldn't push a signup CTA). privacy extends the pattern with `.prose.legal h3` sub-labels, nested `ul ul`, and an `<address class="contact-block">`. Reuse this legal CSS for any future legal page.
...
- **Footer + consent links DONE (was pending):** every footer standardized to **About · Terms · Privacy · Contact** (About→/html/about, Terms→/html/terms, Privacy→/html/privacy, Contact→`mailto:support@realarenas.com`); landing's old `scrollToSection('features')` About bug removed. Signup/auth **consent** links wired to /html/terms & /html/privacy in 4 spots: for-clubs terms-checkbox (`.terms-text` `<span class=terms-link>`→`<a>`), for-clubs launch-consent text, landing-login `.form-terms`, and club-join's JS-built join form.
- **Consent-link gotchas worth keeping:**
  - for-clubs terms checkbox lives inside a `<label class="terms-box">` — converting the inner `<span class="terms-link">` to a real `<a>` is the correct fix: the browser natively does NOT toggle a labeled checkbox when you click an *interactive* `<a>` descendant, so no `stopPropagation` needed. Add `target="_blank" rel="noopener"` so users don't lose the form. `.terms-link` already styles `<a>` (gray-900, underline).
  - **Not every page carries the dual-base `/html`→root rewrite script.** JS-rendered pages like **arenas-club-join.html have NO rewrite script** — hardcoding `/html/...` there would 404 on Railway. They instead expose a `baseUrl` (`var baseUrl = D.baseUrl || ''`); build BASE-aware links as `'<a href="'+esc(baseUrl)+'/terms">'` (mirrors its existing `+esc(baseUrl)+'/landing'` links). Check for the rewrite script before hardcoding `/html/`.
  - landing-login line ~1016 "Sign up free" is `href="#"`+`onclick="setMode('signup');return false"` — a working in-page mode toggle, NOT a dead consent link; leave it.
  - arenas-club-invite.html `.ep-footer` "Privacy policy" `href="#"` is mock **email-preview** chrome (next to dead Unsubscribe/Manage-notifications), not a real signup/consent link — left as-is.
