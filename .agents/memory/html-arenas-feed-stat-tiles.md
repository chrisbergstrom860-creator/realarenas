...
**How to apply:** any new stat surface should reuse the tile pattern + escape values with the page's esc helper (labels too). Empty stats must mean NO row (honest-gap rule). Mobile override specificity: the arenas.css `body:has(.bottom-nav) .ac-stats-row .ac-stat` rule (0-3-1) beats page-level tile modifiers (0-2-0) — text tiles intentionally revert to the shared mobile min-width.
