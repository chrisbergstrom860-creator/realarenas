...
- **PATCH whitelist keyed on start_date**: pre-start everything editable; post-start ONLY title/description — any material key in body → `field_locked`. Derived-done (isExpired || requester's own completion — per-viewer, matching app-wide semantics) → `challenge_ended`, locks even title.
