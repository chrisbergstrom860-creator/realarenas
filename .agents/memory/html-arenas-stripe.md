...
## Club Pro gating gap (mapped July 2026, not yet built)
- Club-side enforcement was deferred in the gating build and never done: free clubs reach Training load, Reports (month+year), and at-risk/nudge — all sold as Club Pro on /for-clubs. Enforcement points: `/api/clubs/:clubId/report`, `/training-load`, `/checkin`, `/nudge-atrisk` (authz is INLINE in handlers, so plan check must be an inline helper after the membership 403, not route middleware).
- **Flag trap:** `PLAN_GATES_ENABLED` is already LIVE in prod — gating club features behind the SAME flag goes live on deploy, not dormant. Independent flip timing needs a separate flag (e.g. CLUB_PLAN_GATES_ENABLED).
- **Mixed endpoint:** `/api/leaderboard/club-dashboard` serves BOTH Starter rankings AND Pro atRisk/atRiskCount — never 403 it; strip the atRisk fields for free clubs (client `if (atRisk.length)` already no-ops safely).
- "Coach notes on events" (Club Pro card claim) does not exist — only a dead mock modal in club-member.html; events' free `description` field is the nearest real thing. Build-or-drop decision required before gating.
- club-dashboard page injects NO gating object today; a `clubProLocked` injection is needed for locked-tab UI (Stats-tab panel treatment + ARL line on its upgrade CTA).
