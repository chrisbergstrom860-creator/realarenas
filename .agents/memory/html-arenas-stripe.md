...
- Plan/price for the email come from `meta.owner_type` (club → Club Pro **$29/month**, else Individual Pro **$9/month**) — same 1:1 owner_type→product mapping the success page uses (distinct prices per flow). **Price strings are HARDCODED in two spots (webhook email builder + success page).** If a Stripe price ever changes, the email will silently state the old price — an ARL-compliance drift; update both, or derive from the Stripe price object.
