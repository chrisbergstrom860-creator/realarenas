...
- Dead-chrome rule enforced again (2026-08): the Discover "type" pill row (solo/community/performance — no such model field, renderDiscover ignored it) and the fake "Load more" toast button are DELETED; sport is the ONLY Discover filter. Header stats strip is fully real; active-count/challenges-available labels are singular/plural-aware spans set by setStat. "challenges available" is now an EXACT head count (one applyDiscoverFilters closure feeds list + count queries; count failure degrades to list length, never fake zero); grid stays capped at 20.

- 2026-08: join/leaderboard now sit on canUserSeeChallenge (see html-arenas-challenge-images.md); stranger denial = byte-identical "Challenge not found"; duplicate join idempotent.
