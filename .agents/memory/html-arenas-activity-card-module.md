...
`html/arenas-activity-card.js` → `window.activityCardBody(a, {inset, title, feeling})` renders the card BODY in the app-wide order **title → stat tiles → notes (→ feeling)**. The old "Coach's note" block (server-canned `ai_insight` templates masquerading as a human coach) was REMOVED by user decision — never reintroduce fabricated coach voice; the DB column `activities.ai_insight` still exists but is unpopulated and `stripLegacyInsight()` in server.js scrubs it from every `select('*')` activity payload (feed, self list, club feed) — keep that on any new activity payload path. Consumers: main feed, club dashboard Feed tab (`{inset:true}` — its card is flush, `.ac-ins` = margin 0 14px), profile Activities tab (`{title:false, feeling:true}` — title lives in its header row). Loads AFTER `arenas-stat-tiles.js`. Body CSS classes (`.ac-title/.ac-notes-box/.fa-notes/.ac-feeling/.ac-ins`) live in `arenas.css`; the old page-local `.fa-notes` copies are deleted.
...
Sport pill: `window.sportPillHtml(sport, {size:'sm'|'xs', icon, style})` in the same module is THE registry-fed pill renderer (colors only from `ARENAS_SPORTS_BY_ID`, no color params; unknown sport → neutral gray + ⚡, never another sport's colors). `xs` = the dashboard's 10px/600/0.5px badge grammar — deliberate, matches its sibling 📣/🎟️ badges, don't converge. The two accent palettes (PROGRESS_COLORS in challenges, CHALLENGE_ACCENTS bars in leaderboards) now cover all 14 sports and were CVD-retinted (lightness spread within each hue family — hue alone collapses for dichromats); ACC bg tile tints are near-identical pales BY DESIGN (emoji carries identity — don't "fix"); guard asserts palette-internal floors normal ΔE≥20 / deutan+protan≥9 / track-visibility≥30 using its own simulate matrix (floors are guard-math values, NOT Viénot values — the two differ, always tune against the guard). verify-sport-colors.js also has a two-tier hardcoded-sport-hex guard: script-context sport+hex lines fail outright; static markup tags (landing/blog mocks — plain sendFile pages, no registry injection) must match current registry hexes or fail; deliberate accent palettes are whitelisted BY NAME (PROGRESS_COLORS, CHALLENGE_ACCENTS) — add a third palette consciously there.

Related facts:
- Club feed shape convergence has a deliberate privacy exception: activity feeling belongs to peer/own views, not club-manager/coach dashboards, analytics, reports, or their payloads. Coaches viewing permitted peer feeds/profiles can still see it.

**Why:** the creator chose surface-based exclusion, not a blanket coach-role restriction. Copy must not promise that coaches can never see feeling.

**How to apply:** describe the excluded surfaces explicitly; retain peer activity-sharing rules and avoid reintroducing the title/notes projection bug when stripping managerial payloads.
